...
class DockerHelper:
    ...
    def build_image(self, dockerfile_name, image_name, tag):
        dockerfile_path = os.path.join(
            self.project_dir, self.dockerdir_rel_path, dockerfile_name
        )
    ...
    def run_docker_compose(
        ...
        if not os.path.isfile(compose_file):
            raise FileNotFoundError(
                f"Docker Compose file ('docker-compose.yml') not found in '{self.dockerdir_rel_path}'"
            )
        # Run Docker Compose project
        containers = self.client.containers.run(
            f"docker/compose:{docker_version}", command=f"-f {compose_file} up -d"
        )
    ...
    def stop_docker_compose(self, project_name="docker_compose_project"):
        ...
        containers = self.client.containers.run(
            "docker/compose:1.29.2", command=f"-f {compose_file} down"
        )
