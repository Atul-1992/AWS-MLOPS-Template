...
class GitHelper:
    ...
    def version_code(self):
        success, output = self.run_git_command(f'git tag -a version -m "{self.version}"')
        if success:
            self.version = self.version + 1
...
class DVCHelper:
    ...
    def dvc_push(self, local_storage_path):
        ...
        success, _ = self.run_command(f"git commit {local_storage_path} -m 'Dataset updates with {self.data_version}'")
        if success==True:
            self.data_version += 1
        return True
    ...
    def create_stage(self, name:str, parameters:list, dependencies:list, outputs:list):
        self.run_command(f"dvc stage add {name} -p {' '.join(parameters)} -d {' '.join(dependencies)} -o {' '.join(outputs)}")
        self.run_command("git add .")
