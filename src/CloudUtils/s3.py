...
class S3Helper(AWSHelper):
    ...
    def download_objects(self, bucket_name, local_dest_path, object_key, version_id=None):
        download_params = {
            'Bucket': bucket_name,
            "Key": object_key,
    ...
    def allow_bucket_download(self, bucket_name, principle_name_ids=None):
        ...
        principal_arns = [f"arn:aws:iam::{account_id}:root" for account_id in principle_name_ids]
    ...
    def list_bucket_versions(self, bucket_name):
        ...
        return {version['VersionId']: version['Key'] for version in response['Versions']}
